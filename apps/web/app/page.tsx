...
import { fetchMyWallet } from '../services/api';
...
export default function Home() {
  ...
  // Balansni yuklab olish — faqat user tayyor bo'lganda
  useEffect(() => {
    if (!user) return;
    fetchMyWallet()
  ...
  // ── Auth yo'q ───────────────────────────────────────────────────────────────
  // Faqat isAuthenticating=false VA user=null bo'lsa ko'rinadi
  if (!user) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4 p-4 text-center">
        <p className="text-muted text-sm">
          Botga kiring va <strong>/start</strong> bosing, so'ng ilovani qayta oching.
        </p>
        {/* Debug: faqat dev da ko'rinadi */}
        {process.env.NODE_ENV !== 'production' && (
          <p className="text-xs text-red-400">
            [dev] user=null, WebApp initData mavjudligini tekshiring
          </p>
        )}
      </div>
    );
