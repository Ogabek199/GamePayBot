  ...
  return (
    <main className="min-h-screen animate-fade-in p-5 pb-32 max-w-md mx-auto space-y-8">
      ...
      <section className="text-center">
         <div className="w-24 h-24 rounded-[2rem] border-4 border-primary/20 p-1 overflow-hidden bg-card mx-auto flex items-center justify-center">
            ...
            {user?.photoUrl && user.photoUrl.trim() !== '' ? (
              <img
                ...
                onError={(e) => {
                  // Agar rasm ssilkasi noto'g'ri bo'lsa yoki yuklanmasa broken image bo'lib qolmasligi uchun
                  (e.target as HTMLImageElement).style.display = 'none';
                }}
              />
            ) : (
              <div className="w-full h-full flex items-center justify-center text-3xl font-bold bg-primary/10 text-primary rounded-[1.5rem]">
                {firstName?.charAt(0) || user?.firstName?.charAt(0) || 'U'}
              </div>
            )}
          </div>
          <p className="text-xs text-muted mt-3 font-medium">Telegram profil rasmi ishlatilmoqda</p>
      </section>

      {/* Ma'lumotlar kiritish maydonlari */}
      <section className="space-y-4">
        {/* Tahrirlanadigan Ism maydoni */}
        <div className="glass-card p-4 flex items-center space-x-4 premium-border">
          ...
          <div className="flex-1">
            ...
            <input
              ...
              className="w-full bg-transparent font-bold text-sm focus:outline-none border-b border-white/10 pb-1 mt-1"
        ...
        <div className="glass-card p-4 flex items-center space-x-4 premium-border opacity-80">
          <div className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center text-primary">
            <Smartphone size={20} />
          </div>
          <div className="flex-1">
        ...
        <div className="glass-card p-4 flex items-center space-x-4 premium-border opacity-80">
          ...
          <div className="flex-1">
            ...
            <p className="font-bold text-sm">{user?.telegramId || 'aniqlanmagan'}</p>
          </div>
        </div>
      </section>
      ...
      <button
        ...
        className="w-full btn-gold h-14 flex items-center justify-center space-x-2 transition-all disabled:opacity-50"
      >
        {loading ? (
